...
class AskRequest(BaseModel):
    ...
    language_filter: Optional[List[str]] = Field(default_factory=lambda: ["en", "hi"], description="Languages to filter")
...
class BharatWitnessAPI:
    ...
    def _initialize_components(self):
        try:
            ...
            indices_loaded = self.index_builder.load_indices()
            
            # Auto-build demo indices if no data exists
            if not indices_loaded or len(self.index_builder.chunk_store) == 0:
                self.logger.info("No indices found, building demo indices with sample data...")
                from pipeline.index_build import build_demo_indices
                build_demo_indices(self.config_path)
                # Reload after building demo indices
                self.index_builder.load_indices()
                self.logger.info("Demo indices built and loaded successfully")
    ...
    async def ask_question(self, request: AskRequest) -> AskResponse:
        ...
        try:
            ...
            # Build answer text from resolved spans
            answer_text = " ".join([span.text for span in resolved_spans[:5]]) if resolved_spans else ""
            
            # Create text spans for verification (if we have content to verify)
            from utils.span_utils import TextSpan
            evidence_text_spans = []
            for span in resolved_spans[:5]:
                text_span = TextSpan(
                    text=span.text,
                    start=0,
                    end=len(span.text),
                    document_id=span.document_id,
                    page_num=span.metadata.get('original_span', {}).get('page_num', 1),
                    section_id=span.metadata.get('document_type', 'unknown'),
                    confidence=span.metadata.get('confidence', 0.8),
                    metadata=span.metadata
                )
                evidence_text_spans.append(text_span)
            
            # Verify answer with actual evidence spans
            verification_summary = self.verifier.verify_answer(answer_text, evidence_text_spans)
