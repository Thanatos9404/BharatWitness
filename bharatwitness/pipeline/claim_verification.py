...
class ClaimVerificationPipeline:
    ...
    def verify_answer(self, answer_text: str, evidence_spans: List[TextSpan]) -> VerificationSummary:
        ...
        if not claims:
            # No claims to verify is acceptable - the answer may still be valid
            return VerificationSummary(
                total_claims=0,
                supported_claims=0,
                refuted_claims=0,
                uncertain_claims=0,
                average_confidence=1.0,  # High confidence when no claims to refute
                verification_threshold=self.verification_threshold,
                refusal_recommended=False  # Don't refuse just because no claims found
            )
